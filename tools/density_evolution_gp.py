...
用法:
  ...
  # 叠加 DFT 计算结果（来自 density.log）
  python density_evolution_gp.py --read-log --dft-log=density.log
...
# ══════════════════════════════════════════════════════════════════
#  解析 density.log（DFT 计算结果）
# ══════════════════════════════════════════════════════════════════

def parse_dft_log(dft_logfile):
    """
    解析 density.log，返回 DFT 计算结果。

    格式: # Crystal_id Density Energy
    例:   267   1.855059 -28081.81189000

    返回: {id: (density_dft, energy_dft)}
    """
    dft_results = {}
    if not exists(dft_logfile):
        print(f"[WARN] DFT log not found: {dft_logfile}")
        return dft_results

    with open(dft_logfile) as f:
        for line in f:
            line = line.strip()
            ...
            try:
                ...
                dft_results[sid] = (density, energy)
            except (ValueError, IndexError):
                continue

    print(f"[DFT] Loaded {len(dft_results)} DFT results from {dft_logfile}")
    return dft_results
...
def plot(x_ticks, ml_best, gene_ml,
    ...
    """单图：ML 密度演化 + GP 预测点（含置信区间） + DFT 计算结果"""
    ...
    # ═══════════════════════════════════════════════════════════
    # DFT 计算结果（来自 density.log）
    # ═══════════════════════════════════════════════════════════
    if dft_results:
        dft_x = []
        dft_y = []
        dft_ids = []
        for sid, (den_dft, eng_dft) in dft_results.items():
            if sid in id_to_info:
                gen_str, ml_d = id_to_info[sid]
                dft_x.append(int(gen_str))
                dft_y.append(den_dft)
                dft_ids.append(sid)

        if dft_x:
            dft_x = np.array(dft_x)
            dft_y = np.array(dft_y)
            dft_ids = np.array(dft_ids)

            ax.scatter(dft_x, dft_y, color='#e74c3c', marker='*', s=180,
                       zorder=5, edgecolors='darkred', linewidth=0.8,
                       label='DFT Density (GGA-PBE)')

            # 标注 DFT 点 ID
            for i in range(len(dft_x)):
                ax.annotate(f'ID:{dft_ids[i]}', (dft_x[i], dft_y[i]),
                            textcoords="offset points", xytext=(0, 14),
                            fontsize=7, color='#c0392b', ha='center',
                            alpha=0.85, fontweight='bold')

            # DFT 最高密度标注
            idx_dft = np.argmax(dft_y)
            ax.annotate(f'DFT max: {dft_y[idx_dft]:.3f} @ Gen {dft_x[idx_dft]} (ID:{dft_ids[idx_dft]})',
                        xy=(dft_x[idx_dft], dft_y[idx_dft]),
                        xytext=(dft_x[idx_dft] - 2.5, dft_y[idx_dft] - 0.04),
                        arrowprops=dict(arrowstyle='->', color='#e74c3c', lw=1.5),
                        fontsize=10, color='#e74c3c', fontweight='bold')

            # DFT 统计
            print(f"\n{'='*60}")
            print(f"  DFT 计算结果 ({len(dft_x)} 个结构)")
            print(f"{'='*60}")
            print(f"  DFT density:  min={dft_y.min():.4f}  max={dft_y.max():.4f}  "
                  f"mean={dft_y.mean():.4f}")
            print(f"  {'ID':>6} {'Gen':>4} {'DFT':>8}")
            print(f"  {'-'*22}")
            for i in range(len(dft_x)):
                print(f"  {dft_ids[i]!s:>6} {dft_x[i]:4d} {dft_y[i]:8.3f}")

            # 扩展 Y 轴范围以包含 DFT 点
            current_ylim = ax.get_ylim()
            dft_min = dft_y.min() - 0.03
            dft_max = dft_y.max() + 0.04
            ax.set_ylim(min(current_ylim[0], dft_min),
                        max(current_ylim[1], dft_max))

    ax.set_xlabel('Generation', fontsize=13)
    ax.set_ylabel(r'Density (g/cm$^3$)', fontsize=13)
    ax.set_title(r'USPEX Density Evolution — ML + GP Prediction + DFT  |  '
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dft-log', default=None,
                        help='DFT 计算结果 density.log 文件路径 '
                             '(default: 不绘制 DFT 数据)')
    ...
    dft_results = {}
    if args.dft_log:
        dft_results = parse_dft_log(args.dft_log)
